      ...
      {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((d) => {
        ...
        return (
          <button
            ...
            onClick={() => onSelect(d)}
            aria-label={`输入 ${d}`}
            className={`relative flex h-10 flex-col items-center justify-center rounded-xl border text-lg font-semibold transition-all duration-150 active:scale-95 sm:h-16 sm:text-xl ${
              done
